...
void draw_tiles(Texture2D *tile, Texture2D *blank_tile, Texture2D *flag_tile,
                Texture2D *mine_tile);
int main(void) {
    ...
    Image blank_tile_img =
        LoadImageFromMemory(".png", BLANK_TILE_PNG, ARRAYSIZE(BLANK_TILE_PNG));
    Texture2D blank_tile = LoadTextureFromImage(blank_tile_img);

    Image flag_tile_img =
        LoadImageFromMemory(".png", FLAG_TILE_PNG, ARRAYSIZE(FLAG_TILE_PNG));
    Texture2D flag_tile = LoadTextureFromImage(flag_tile_img);

    Image mine_tile_img =
        LoadImageFromMemory(".png", MINE_TILE_PNG, ARRAYSIZE(MINE_TILE_PNG));
...
void generate_mines(void) {
    for (unsigned char i = 0; i < MINE_COUNT; ++i) {
        while (1) {
            ...
            if (GAME.tiles[idx].type == TILE_TYPE_CLOSE &&
                !GAME.tiles[idx].mine) {
    ...
    for (unsigned char x = 0; x < BOARD_SIZE; ++x) {
        for (unsigned char y = 0; y < BOARD_SIZE; ++y) {
            ...
            for (signed char x_off = start_x_off; x_off <= end_x_off; ++x_off) {
                for (signed char y_off = start_y_off; y_off <= end_y_off;
                     ++y_off) {
                    unsigned char off_idx =
                        (y + y_off) * BOARD_SIZE + (x + x_off);
...
void flood_fill(unsigned char idx) {
    ...
    if (idx >= BOARD_SIZE) {
        ...
        if ((!requires_zero_nearby || tt->nearby_mines == 0) &&
            tt->type == TILE_TYPE_CLOSE) {
...
void input(unsigned char idx, enum tile_type type, bool chord);
void handle_inputs(void) {
    ...
    if ((IsMouseButtonPressed(MOUSE_BUTTON_LEFT) ||
        ...
        !GAME.game_over) {
        enum tile_type type = IsMouseButtonPressed(MOUSE_BUTTON_LEFT)
                                  ? TILE_TYPE_OPEN
                                  : TILE_TYPE_FLAG;

        input(idx, type, false);
    }
}
...
void input(unsigned char idx, enum tile_type type, bool chord) {
    ...
    if (GAME.tiles[idx].type == TILE_TYPE_FLAG && chord) {
        return;
    }

    if (GAME.tiles[idx].type == TILE_TYPE_OPEN && type == TILE_TYPE_OPEN &&
        !chord) {
        // Attempt to chord the cell
        bool up = idx >= BOARD_SIZE;
        bool down = idx / BOARD_SIZE != BOARD_SIZE - 1;
        bool left = idx % BOARD_SIZE != 0;
        bool right = idx % BOARD_SIZE != (BOARD_SIZE - 1);

        unsigned char indicies[8] = {0};
        unsigned char len = 0;

        // Create array of tile indicies where the chord would apply
        if (up) {
            indicies[len++] = idx - BOARD_SIZE;
            if (left) indicies[len++] = idx - BOARD_SIZE - 1;
            if (right) indicies[len++] = idx - BOARD_SIZE + 1;
        }
        ...
        if (left) indicies[len++] = idx - 1;
        if (right) indicies[len++] = idx + 1;

        // Count how many chorded cells are flagged
        unsigned char num_flags = 0;
        for (unsigned char i = 0; i < len; ++i) {
            if (GAME.tiles[indicies[i]].type == TILE_TYPE_FLAG) {
                ++num_flags;
            }
        }

        // Only chord if no. flags is greater than or equal to the no. nearby
        // mines
        if (num_flags >= GAME.tiles[idx].nearby_mines) {
            for (unsigned char i = 0; i < len; ++i) {
                input(indicies[i], TILE_TYPE_OPEN, true);
            }
        }
    }

    if (GAME.tiles[idx].type == TILE_TYPE_FLAG && type == TILE_TYPE_FLAG) {
        GAME.tiles[idx].type = TILE_TYPE_CLOSE;
        ++GAME.num_flags;
    } else if (!(GAME.tiles[idx].type == TILE_TYPE_OPEN &&
                 type == TILE_TYPE_FLAG)) {
        if (type == TILE_TYPE_FLAG && GAME.tiles[idx].type != TILE_TYPE_FLAG) {
            --GAME.num_flags;
        } else if (type == TILE_TYPE_OPEN &&
                   GAME.tiles[idx].type == TILE_TYPE_FLAG) {
            ++GAME.num_flags;
        }
        ...
        // Check to see if player won, zero remaining flags, with all flags
        // on mines
        if (GAME.num_flags == 0 && type == TILE_TYPE_FLAG) {
            ...
            for (unsigned short i = 0; i < (BOARD_SIZE * BOARD_SIZE); ++i) {
                if (GAME.tiles[i].mine &&
                    GAME.tiles[i].type != TILE_TYPE_FLAG) {
                    GAME.won = false;
...
void draw_menu(Texture2D *flag) {
    ...
    const char *reset_text =
        GAME.mines_generated ? "'R' to reset." : "by jayphen";

    DrawText(reset_text, 5 + 2, 36 + 2, 15,
             GAME.game_over ? ColorAlpha(GAME.won ? BLACK : BLACK,
                                         0.2f * sinf(GetTime() * 20.0f) + 0.8f)
                            : BLACK);
    DrawText(reset_text, 5, 36, 15,
             GAME.game_over ? ColorAlpha(GAME.won ? GREEN : YELLOW,
                                         0.2f * sinf(GetTime() * 20.0f) + 0.8f)
                            : WHITE);
    ...
    unsigned int total_seconds_played =
        (unsigned int)(GAME.time_end - GAME.time_start);
    ...
    for (unsigned char x = 0; x < BOARD_SIZE; ++x) {
        ...
        for (unsigned char y = 0; y < BOARD_SIZE; ++y) {
            unsigned int y_offset =
                TITLE_HEIGHT + (GAME_WIDTH / BOARD_SIZE) * y;
            unsigned char idx = (y * BOARD_SIZE) + x;
            struct tile *t = &GAME.tiles[idx];
            Texture2D *tex = NULL;
            ...
            DrawTextureEx(*tex, (Vector2){x_offset, y_offset}, 0.0f, 2.0f,
                          WHITE);
            if (should_draw_text && t->nearby_mines != 0) {
                ...
                DrawText(buff, x_offset + text_offset + 2, y_offset + 1 + 2, 30,
                         BLACK);
